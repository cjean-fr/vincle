...
describe("renderToStatic", () => {
  ...
  describe("SSG stress — concurrency (regression: initFlowAssets race)", () => {
    ...
    it("renders many pages with fragments concurrently", async () => {
      const pages = await Promise.all(
        Array.from({ length: PAGE_COUNT }, (_, i) =>
          renderToStatic(
            async (ctx) => {
              const AsyncContent = async () => (<span>frag-{i}</span>) as VNode;
              const html = await ctx.renderPage(() => (
                <html>
                  <body>
                    <p>page-{i}</p>
    ...
    it("handles mixed load with and without fragments, high concurrency", async () => {
      const [purePages, fragmentPages]: [string[], Array<{ html: string; ids: string[] }>] =
        await Promise.all([
          Promise.all(
          ...
          Promise.all(
            Array.from({ length: PAGE_COUNT / 2 }, (_, k) => {
              ...
              return renderToStatic(
                async (ctx) => {
                  const AsyncContent = async () => (<span>odd-{i}</span>) as VNode;
                  const html = await ctx.renderPage(() => (
                    <html>
                      <body>
                        <p>page-{i}</p>
      ...
      for (let k = 0; k < PAGE_COUNT / 2; k++) {
        ...
        const iEven = k * 2;
        const iOdd = k * 2 + 1;
        expect(pure).toContain(`<p>page-${iEven}</p>`);
