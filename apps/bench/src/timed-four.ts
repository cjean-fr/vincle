// ── Benchmark 4-way (kita, core-next, preact, vincle-core) ─────────
...
import { render as renderVincle } from "./realworld/vincle-core.js";
...
for (let s = 0; s < SAMPLES; s++) {
  ...
  for (let b = 0; b < BLOCKS; b++) {
    ...
    t = performance.now();
    for (let i = 0; i < BLOCK; i++) renderVincle(NAME, PURCHASES);
    localVincle.push(performance.now() - t);
  ...
  rVincle.push(localVincle.reduce((a, b) => a + b, 0));
}
...
const sVincle = stats(rVincle);

console.log(`Benchmark 4-way (${BLOCK} iters × ${BLOCKS} blocks × ${SAMPLES} samples)`);
console.log(`cooling: 1500ms entre chaque block`);
console.log(``);
console.log(`kita-html:`);
console.log(`  wall:    min=${sKita.min.toFixed(0)}ms  med=${sKita.med.toFixed(0)}ms  max=${sKita.max.toFixed(0)}ms  spread=${sKita.spread.toFixed(1)}%`);
console.log(`core-next:`);
console.log(`  wall:    min=${sNext.min.toFixed(0)}ms  med=${sNext.med.toFixed(0)}ms  max=${sNext.max.toFixed(0)}ms  spread=${sNext.spread.toFixed(1)}%`);
console.log(`preact:`);
console.log(`  wall:    min=${sPreact.min.toFixed(0)}ms  med=${sPreact.med.toFixed(0)}ms  max=${sPreact.max.toFixed(0)}ms  spread=${sPreact.spread.toFixed(1)}%`);
console.log(`vincle-core:`);
console.log(`  wall:    min=${sVincle.min.toFixed(0)}ms  med=${sVincle.med.toFixed(0)}ms  max=${sVincle.max.toFixed(0)}ms  spread=${sVincle.spread.toFixed(1)}%`);
console.log(``);
console.log(`ratio (med): core-next/kita      = ${(sNext.med / sKita.med).toFixed(3)}×`);
console.log(`ratio (med): vincle-core/kita    = ${(sVincle.med / sKita.med).toFixed(3)}×`);
console.log(`ratio (med): preact/kita         = ${(sPreact.med / sKita.med).toFixed(3)}×`);
console.log(`ratio (med): core-next/vincle    = ${(sNext.med / sVincle.med).toFixed(3)}×`);
console.log(``);
...
console.log(`Échantillons vincle:  [${rVincle.map(v => v.toFixed(0)).join(", ")}]`);
