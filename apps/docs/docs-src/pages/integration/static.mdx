...
`{ url, html }`. How you publish those bytes at `url`, whether through a blob
store, a revalidation call, or a CDN purge and re-upload, depends on the host and stays in
