...
| `onError`        | `(error, info) => JSX.Element \| void` | —        | Per-fragment error handler        |
...
When `mode` is set in both `options` and the negotiation result, the value
passed to `serve()` wins — `opts.mode ?? negotiation.mode` — so options
override whatever the request header suggests.
