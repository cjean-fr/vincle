...
|   5 | Build-time Sanitization | `@vincle/vite-plugin-precompile` defaults to       | Literal `javascript:` and unsafe `data:` URLs —         |
|     |                         | sanitizing — static attributes run through         | at build time, before the browser parses any            |
|     |                         | the same `jsxAttr` as dynamic values, at build     | HTML                                                    |
|     |                         | time, not at runtime                               |                                                         |
