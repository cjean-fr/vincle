...
public API. `isSafeScheme`, for example, is not exported from `@vincle/core` —
the public export is `schemeOf`.
...
| Every attribute value, including `on*`                            | `<iframe srcdoc={untrusted}>` — the value is decoded *and then parsed as a document*, so escaping it is not enough |
...
[^js]: The element boundary, in the form the sub-language reads back. The
    JavaScript *string* context is the author's: serialize untrusted data with
