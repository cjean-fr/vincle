...
Vincle's `createContext` and `<Context.Provider>` scope a value to a subtree,
as in Preact, Hono and React. Its separate `ExecutionContext` stores mutable
per-execution state that later siblings can read. Both retain isolation across
concurrent renders. See the [Context API](/api/core/context).
