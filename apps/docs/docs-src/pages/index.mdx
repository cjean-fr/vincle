...
description: "Compose typed JSX views in your existing server. Add progressive HTML fragments when your page needs them."
prose: false
---
...
<section class="docs-home-section" aria-labelledby="use-cases-title">
  ...
  <section class="docs-home-section docs-foundations" aria-labelledby="streaming-title">
    <div>
      ...
      <p>{"Keep the heading visible while an async component loads. With flow, a fallback fills the space until its HTML is ready."}</p>
      <p>{"Choose an adapter for delivery, including HTMX or Turbo. Browser updates use the chosen adapter’s client mechanism."}</p>
      <a class="docs-text-link" href="/guide/views">
  ...
  <section class="docs-home-section docs-foundations" aria-labelledby="features-title">
    ...
    <div class="docs-feature-list">
      <a href="/guide/jsx/components" class="docs-feature-link">
        <div>
          <h3>{"Async components"}</h3>
          <p>{"Fetch data inside your components. Await the tree. Vincle handles the rendering."}</p>
        </div>
        <span aria-hidden="true">{"↗"}</span>
      </a>
      <a href="/api/core/scope" class="docs-feature-link">
        <div>
          <h3>{"Tree context"}</h3>
          <p>{"Share a value with a subtree through a React-style context Provider — or keep per-request state in a Scope."}</p>
        </div>
        <span aria-hidden="true">{"↗"}</span>
      </a>
      <a href="/guide/security" class="docs-feature-link">
        <div>
          <h3>{"HTML escaping"}</h3>
          <p>{"Content is escaped by default, dangerous URL schemes are blocked, and event handlers stay out of HTML."}</p>
        </div>
        <span aria-hidden="true">{"↗"}</span>
      </a>
      <a href="/guide/jsx/elements-attributes" class="docs-feature-link">
        <div>
          <h3>{"TypeScript support"}</h3>
          <p>{"Write components and HTML attributes with TypeScript checking your work."}</p>
        </div>
        <span aria-hidden="true">{"↗"}</span>
      </a>
    </div>
